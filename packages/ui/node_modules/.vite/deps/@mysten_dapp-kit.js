...
var require_with_selector_development = __commonJS({
  "../../node_modules/use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js"(exports) {
    ...
    (function() {
      ...
      var React = require_react(), shim = require_shim(), objectIs = "function" === typeof Object.is ? Object.is : is, useSyncExternalStore = shim.useSyncExternalStore, useRef2 = React.useRef, useEffect4 = React.useEffect, useMemo3 = React.useMemo, useDebugValue2 = React.useDebugValue;
...
function registerSlushWallet(name, {
  ...
  const extension = wallets2.get().find((wallet) => wallet.id === SUI_WALLET_EXTENSION_ID);
  if (extension) {
    return;
  }
...
setupWebSocket_fn = function() {
  ...
  __privateSet3(this, _connectionPromise, new Promise((resolve) => {
    __privateGet3(this, _webSocket)?.close();
...
var JsonRpcHTTPTransport = class {
  constructor(options) {
    __privateAdd4(this, _JsonRpcHTTPTransport_instances);
  ...
  fetch(input, init) {
    const fetchFn = __privateGet4(this, _options).fetch ?? fetch;
    if (!fetchFn) {
      throw new Error(
  ...
  async request(input) {
    ...
    const res = await this.fetch(__privateGet4(this, _options).rpc?.url ?? __privateGet4(this, _options).url, {
      method: "POST",
      signal: input.signal,
      headers: {
        ...
        ...__privateGet4(this, _options).rpc?.headers
      },
      body: JSON.stringify({
        jsonrpc: "2.0",
  ...
  async subscribe(input) {
    const unsubscribe = await __privateMethod4(this, _JsonRpcHTTPTransport_instances, getWebsocketClient_fn).call(this).subscribe(input);
    if (input.signal) {
...
getWebsocketClient_fn = function() {
  if (!__privateGet4(this, _websocketClient)) {
    const WebSocketConstructor = __privateGet4(this, _options).WebSocketConstructor ?? WebSocket;
    ...
    __privateSet4(this, _websocketClient, new WebsocketClient(
      __privateGet4(this, _options).websocket?.url ?? __privateGet4(this, _options).url,
      {
...
var JSONRpcCoreClient = class extends Experimental_CoreClient {
  constructor({
    ...
    __privateAdd5(this, _jsonRpcClient);
    __privateSet5(this, _jsonRpcClient, jsonRpcClient);
  }
  async getObjects(options) {
    const batches = chunk(options.objectIds, 50);
    const results = [];
    for (const batch of batches) {
      const objects = await __privateGet5(this, _jsonRpcClient).multiGetObjects({
        ids: batch,
        options: {
          showOwner: true,
          showType: true,
  ...
  async getOwnedObjects(options) {
    const objects = await __privateGet5(this, _jsonRpcClient).getOwnedObjects({
  ...
  async getCoins(options) {
    const coins = await __privateGet5(this, _jsonRpcClient).getCoins({
      owner: options.address,
  ...
  async getBalance(options) {
    const balance = await __privateGet5(this, _jsonRpcClient).getBalance({
      owner: options.address,
      coinType: options.coinType,
      signal: options.signal
    });
  ...
  async getAllBalances(options) {
    const balances = await __privateGet5(this, _jsonRpcClient).getAllBalances({
      owner: options.address,
      signal: options.signal
    });
    return {
  ...
  async getTransaction(options) {
    const transaction = await __privateGet5(this, _jsonRpcClient).getTransactionBlock({
  ...
  async executeTransaction(options) {
    const transaction = await __privateGet5(this, _jsonRpcClient).executeTransactionBlock({
  ...
  async dryRunTransaction(options) {
    ...
    const result = await __privateGet5(this, _jsonRpcClient).dryRunTransactionBlock({
      transactionBlock: options.transaction,
      signal: options.signal
    });
  ...
  async getReferenceGasPrice(options) {
    const referenceGasPrice = await __privateGet5(this, _jsonRpcClient).getReferenceGasPrice({
      signal: options?.signal
    });
    return {
  ...
  async getDynamicFields(options) {
    const dynamicFields = await __privateGet5(this, _jsonRpcClient).getDynamicFields({
  ...
  async verifyZkLoginSignature(options) {
    const result = await __privateGet5(this, _jsonRpcClient).verifyZkLoginSignature({
      bytes: options.bytes,
  ...
  resolveNameServiceNames(options) {
    return __privateGet5(this, _jsonRpcClient).resolveNameServiceNames(options);
  }
  resolveTransactionPlugin() {
    return jsonRpcClientResolveTransactionPlugin(__privateGet5(this, _jsonRpcClient));
  }
  async getMoveFunction(options) {
    const result = await __privateGet5(this, _jsonRpcClient).getNormalizedMoveFunction({
...
function registerUnsafeBurnerWallet(suiClient) {
  ...
  class UnsafeBurnerWallet {
    constructor() {
      ...
      __privateAdd6(this, _signAndExecuteTransaction2, async (transactionInput) => {
        const { bytes, signature } = await Transaction.from(
          await transactionInput.transaction.toJSON()
        ).sign({
          client: suiClient,
    ...
    get features() {
      return {
        ...
        "sui:signAndExecuteTransaction": {
          version: "2.0.0",
          signAndExecuteTransaction: __privateGet6(this, _signAndExecuteTransaction2)
        }
      };
    }
  }
